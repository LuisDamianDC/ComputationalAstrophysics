...
aumento=10                                     #Aumento de velocidad en las
                                                #
...
class Ball():
    ...
    def move(self, pelotas, dt=0.01):
        ...
        if (self.y < self.r or self.y > HEIGHT-self.r):
            self.x=np.random.uniform(0, WIDTH)    # Pos X
            self.y=np.random.uniform(0, HEIGHT)   # Pos y
            self.v_x=np.random.uniform(-50,50)       # Vel x
            self.v_y=np.random.uniform(-50,50)        # Vel y
        # Choque contra las paredes
        if self.x < self.r:
            self.v_x = - self.v_x+aumento
            self.x = self.r
